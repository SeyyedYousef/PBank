...
// ANIMATED COUNTER
// ═══════════════════════════════════════════════
const AnimatedCounter = ({ value, visible }: { value: number; visible: boolean }) => {
    ...
    return <span className="tabular-nums ltr-nums">{display.toLocaleString()}</span>;
};

// ═══════════════════════════════════════════════
// PROGRESS RING
// ═══════════════════════════════════════════════
const ProgressRing = ({ percent, size, stroke, color, children }: { percent: number; size: number; stroke: number; color: string; children?: React.ReactNode }) => {
...
// SECURITY TOGGLE
// ═══════════════════════════════════════════════
const SecurityToggleSwitch = ({ enabled, onToggle, color = 'emerald' }: { enabled: boolean; onToggle: () => void; color?: string }) => {
    ...
    return (
        ...
        >
            <div className={`omega-toggle relative w-[52px] h-[28px] rounded-full p-[3px] transition-all duration-400 ${enabled ? `${colors[color]} shadow-[0_0_20px_rgba(0,255,148,0.3)]` : 'bg-white/10'
                }`} onClick={onToggle}>
                <motion.div
                    className="w-[22px] h-[22px] rounded-full bg-white shadow-lg"
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-white">
                                        <AnimatedCounter value={balance} visible={!isPrivacyMode} />
                                    </span>
